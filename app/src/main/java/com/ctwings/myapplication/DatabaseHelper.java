...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public static final String RECORD_LICENSE_PLATE = "record_license_plate";

    //plates
    public static final String PLATE_ID = "plate_id";
    public static final String PLATE_LICENSE = "plate_license";
    ...
    private static final String[] PLATES_COLUMNS = {PLATE_ID, PLATE_LICENSE};
    ...
    String CREATE_RECORD_TABLE = "CREATE TABLE IF NOT EXISTS " + TABLE_RECORD + " ( " +
            ...
            RECORD_DATE + " INTEGER, " +
            RECORD_SYNC + " INTEGER, " +
            RECORD_LICENSE_PLATE + " TEXT)";

    String CREATE_PLATES_LICENCE = "CREATE TABLE IF NOT EXISTS " + TABLE_PLATES + " ( " +
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL(CREATE_PLATES_LICENCE);
     ...
     *
     * @param json Rut: Rut or Passport Number.
    ...
    public void add_record(Record record) {
        ...
        values.put(RECORD_LICENSE_PLATE, record.getRecord_license_plate());
        ...
        try {
            ...
            while (!cursor.isAfterLast()) {
                try {
                    ...
                    //record.setRecord_license_plate(cursor.getString(cursor.getColumnIndex(RECORD_LICENSE_PLATE)));
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
                    records.add(record);
    ...
    public List<Record> getRecordsDump() {
        ...
        try {
            ...
            while (!cursor.isAfterLast()) {
                ...
                record.setPerson_mongo_id(cursor.getString(1));         // FULLNAME
                record.setRecord_person_rut(cursor.getString(2));       // RUN
                record.setRecord_type(cursor.getString(3));             // TYPE
                record.setRecord_date(cursor.getInt(4));                // DATE
                record.setRecord_sync(cursor.getInt(5));                // SYNC
                records.add(record);
                cursor.moveToNext();
            }
        } catch (SQLException e) {
    ...
    public ArrayList<String> getPlatesFromDb(String searchPlate) {
        SQLiteDatabase db = this.getReadableDatabase();
        ArrayList<String> plates = new ArrayList<>();
        Cursor cursor = null;
        try {
            cursor = db.rawQuery("SELECT plate_license FROM PLATES WHERE plate_license like '" + searchPlate + "%'", null);
            cursor.moveToFirst();
            while (!cursor.isAfterLast()) {
                try {
                    plates.add(cursor.getString(cursor.getColumnIndex(PLATE_LICENSE)));
                } catch (Exception e) {
                    e.printStackTrace();
                }
                cursor.moveToNext();
